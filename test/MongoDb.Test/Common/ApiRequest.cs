...
public class Test_ApiRequest
{
    ...
    {
        var ar = new ApiRequest(new string[] { "id", "name" }, new Filter[] { new Filter("id", FieldOperator.Equal, "1") }, new Order[] { new Order("id", Direction.Ascending) }, new PageContext(10, 100));
        Assert.NotNull(ar);
        Assert.Equal(new string[] { "id", "name" }, ar.Select);
        Assert.Equal("id", ar.Where?.First()?.FieldFilter?.Field);
